...
//  NewsApp
//
//  Created by Alina Costache on 10/28/16.
...
class ArticlesViewController: ArticleViewController, UISearchBarDelegate {
    ...
    func setGestureRecognizers() {
        self.gestureLeft = UIScreenEdgePanGestureRecognizer(target: self, action: #selector(ArticlesViewController.handleTap(_:)))
    ...
    func markUnmarkArticle(_ sender: UIButton) {
        ...
        if sender.accessibilityLabel == "articlesForDisplay" {
            articleToSave = Article(source: self.articlesForDisplay[row].source,
                                    category: self.articlesForDisplay[row].category,
                                    sourceImage: self.articlesForDisplay[row].sourceImage,
            ...
            articleToSave = Article(source: self.filteredArticles[row].source,
                                    category: self.filteredArticles[row].category,
                                    sourceImage: self.filteredArticles[row].sourceImage,
    ...
    override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        //Set the content of the title, description and sourceName labels.
        if articlesToDisplay[indexPath.row].title != "" {
            cell.titleLabel.text = articlesToDisplay[indexPath.row].title
        } else {
            cell.titleLabel.text = "No title."
            cell.titleLabel.font = UIFont(name: "Roboto-Italic" , size: 16)
        }
        cell.sourceName.text = articlesToDisplay[indexPath.row].source
        if self.gridDisplay == false {
            if articlesToDisplay[indexPath.row].articleDescription != "" {
                ...
                cell.descriptionLabel.font = UIFont(name: "Roboto-Italic" , size: 16)
            }
        }
        ...
        //Download and set the source image.
        let sourceImageString : String = articlesToDisplay[indexPath.row].sourceImage
        if  URL(string: sourceImageString) != nil {
            cell.sourceImage.contentMode = .scaleAspectFit
            cell.sourceImage.sd_setImage(with: URL(string: sourceImageString), placeholderImage:UIImage(contentsOfFile:"background_card_big"), options: [.continueInBackground])
            cell.sourceImage.layer.cornerRadius = 2.0
            cell.sourceImage.layer.masksToBounds = true
        }
        ...
        {
            cell.addToArticlesList.addTarget(self, action: #selector(ArticlesViewController.removeArticle(_:)), for: UIControlEvents.touchUpInside)
            ...
            cell.addToArticlesList.addTarget(self, action: #selector(ArticlesViewController.markUnmarkArticle(_:)), for: UIControlEvents.touchUpInside)
            cell.addToArticlesList.tag = indexPath.row
            
         
        }
        
        cell.shareButton.addTarget(self, action:  #selector(ArticlesViewController.share(_:)), for: UIControlEvents.touchUpInside)
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        let secondViewController = UIStoryboard(name: "Main", bundle: nil).instantiateViewController(withIdentifier: "FullArticleViewController") as! FullArticleViewController
